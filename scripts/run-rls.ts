/**
 * Supabase RLS 마이그레이션 실행 스크립트
 *
 * 이 스크립트는 Supabase Management API를 사용하여
 * RLS (Row Level Security) 마이그레이션 파일만 실행합니다.
 *
 * 사용법:
 *   yarn db:run-rls
 *
...
async function main() {
  try {
    console.log("🚀 Supabase RLS 마이그레이션 시작...\n");
    ...
    console.log(`📄 마이그레이션 파일: 0002_enable_rls_and_policies.sql`);
    ...
    console.log(`✅ 0002_enable_rls_and_policies.sql 실행 완료\n`);

    console.log("🎉 RLS 마이그레이션이 성공적으로 완료되었습니다!");
  } catch (error) {
    console.error("\n❌ 마이그레이션 실행 중 오류 발생:");
