/**
 * payments 테이블: 본인 주문에 한해 UPDATE 허용 정책 추가
 *
 * 결제 승인/실패 시 주문 소유자가 해당 주문의 payments 행을 UPDATE할 수 있도록
 * payments_update_own_order 정책을 생성합니다.
 *
 * 사용법:
 *   yarn db:allow-payments-update-own-order
 */
...
async function main() {
  try {
    console.log(
      "🚀 payments UPDATE 본인 주문 허용 정책 마이그레이션 실행...\n"
    );
    ...
    const migrationPath = join(
      ...
      "supabase",
      "migrations",
      "0011_allow_payments_update_own_order.sql"
    );
    ...
    console.log("✅ 0011_allow_payments_update_own_order.sql 실행 완료\n");
    console.log(
      "🎉 결제 승인/실패 시 본인 주문의 payments UPDATE가 허용되었습니다."
    );
  } catch (error) {
    console.error("\n❌ 마이그레이션 실행 중 오류:");
